...
import 'package:flutter_app_backend/components/text_widget.dart';
import 'package:flutter_app_backend/models/get_article_info.dart';
import 'package:flutter_app_backend/models/get_about_info.dart';
import 'package:flutter_app_backend/models/get_workshop_info.dart';
...
class WorkshopDetailsPage extends StatefulWidget {
    ...
    final int index;
  WorkshopDetailsPage({Key key, this.workshopInfo, this.index})
      : super(key: key);
...
/*
  ...
  _initData() async {
    await CallApi().getPublicData("allworkshop").then((response) {
      setState(() {
        Iterable list = json.decode(response.body);
        workshopInfo = list.map((model) => WorkshopInfo.fromJson(model)).toList();
      });
    });
  }
  @override
    ...
    return Container(
      ...
      child: SafeArea(
        child: Scaffold(
            ...
            appBar: AppBar(
              toolbarHeight: 20,
              title: Text(this.widget.workshopInfo.title),
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        TextWidget(
                          text: this.widget.workshopInfo.title,
                          fontSize: 20,
                 ...
                 //   padding: const EdgeInsets.only(right: 20, left:20),
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        children: [
                          TextWidget(
                            text: this.widget.workshopInfo.about_content,
                            fontSize: 16,
                            color: Colors.black,
                          ),
                          Image.network(
                            "http://192.168.46.251/uploads/" +
                                this.widget.workshopInfo.img.toString(),
                            fit: BoxFit.contain,
                          ),
                        ],
                      ),

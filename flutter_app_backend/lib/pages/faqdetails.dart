...
import 'package:flutter_app_backend/components/text_widget.dart';
import 'package:flutter_app_backend/models/get_article_info.dart';
import 'package:flutter_app_backend/models/get_faq_info.dart';
...
class FAQDetailsPage extends StatefulWidget {
  ...
  const FAQDetailsPage({Key key}) : super(key: key);
...
/*
  ...
  _initData() async {
    await CallApi().getPublicData("allfaq").then((response) {
      setState(() {
        Iterable list = json.decode(response.body);
        faqInfo = list.map((model) => FAQInfo.fromJson(model)).toList();
      });
    });
  }
  @override
                 ...
                 //   padding: const EdgeInsets.only(right: 20, left:20),
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        children: [
                          TextWidget(
                            text: this.faqInfo[0].about_content,
                            fontSize: 16,
                            color: Colors.black,
                          ),
                          Image.network(
                            "http://192.168.46.251/uploads/" +
                                this.faqInfo[0].img.toString(),
                            fit: BoxFit.contain,
                          ),
                        ],
                      ),

...
class SkillsdetailPage extends StatefulWidget {
  ...
  final int index;
  SkillsdetailPage({Key key, this.remedialInfo, this.index})
      : super(key: key);
  ...
  _initData() async {
    ...
    await CallApi().getPublicData("allarticles").then((response) {
      setState(() {
        ...
        allarticles = list.map((model) => RemedialInfo.fromJson(model)).toList();
      });
    });
    ...
    return Container(
      ...
      child: SafeArea(
        child: Scaffold(
            ...
            appBar: AppBar(
              toolbarHeight: 20,
              title: Text(this.widget.remedialInfo.title),
                        ...
                        children: [
                          ...
                          Image.network(
                            "http://192.168.46.251/uploads/" +
                                this.widget.remedialInfo.img.toString(),
                            fit: BoxFit.contain,
                          ),
                        ],
                      ),
